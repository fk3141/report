...
\title{Inorganic and Analyutical experiment unit.1}
\date{\today}
\author{Author: No.7 05253011 Fumiya Kashiwai / 柏井史哉}
\begin{document}
\maketitle
\markboth{Inorganic and Analytical experiment Unit.1   No.7 05253011 Fumiya Kashiwai / 柏井史哉} {Inorganic and Analytical experiment Unit.1   No.7 05253011 Fumiya Kashiwai / 柏井史哉}
%%ここまでタイトル

\newpage
\section{Purpose}
GM管の特性曲線を測定することにより、適当な測定電圧を決定する。

\section{Experimental}
\begin{enumerate}
    \item GM管の電源を入れ、安定するまで10 min程度まった。
    \item \ce{^14C}線源をGMスタンドの上から2段目にセットし、電圧を少しずつあげた。計数が動き出した電圧を始動電圧として記録した。
    \item 始動電圧において、1 min計数値を測定した。
    \item 約50 Vずつ印加電圧を上げながら、各電圧値において1 min計数値を測定した。
    \item 1400 Vまで記録し、特性曲線を作成した。
\end{enumerate}

\section{Results}
計測結果は、次の図\ref{gm_tube}のようになった。
\begin{figure}[htbp]
\begin{center}
...
測定電圧として、1000 Vと 1300 Vを1:2に内分する、1100 Vに決定した。

\section{Discussion}
\subsection{課題1: 特性曲線の形}
GM管では、ベータ線またはガンマ線が入射すると、内部のガス分子を電離させ、さらに雪崩的に電離させることで電流が流れ、これにより入射したcount数を測定している。

始動領域では、電圧を上げるに従って電子雪崩の生じる規模が大きくなり、入射放射線の測定効率が上昇する (より低エネルギーの放射線も観測できるようになる) ため、電圧を上げるに従ってcount数が上昇する。

さらに電圧を上昇させると、電子雪崩が計測管全体に生じる、ガイガー放電となる。その場合、入射放射線のエネルギーに依らずに測定できるようになり、プラトー領域に達する。

さらに電圧を上げると、放射線が入射せずとも自己放電が開始する。この場合、電圧を上げるほどに計数率が上昇する。

\subsection{課題2: 不感時間について}
一度上述の電子雪崩が発生すると、移動度の低いカチオンが緩和するまで、電界を弱めるために電子雪崩が生じなくなり。放射線が入射しても観測することができなくなる。この時間を不感時間と呼び、強すぎる放射線を測定すると精度が低下することにつながる。
