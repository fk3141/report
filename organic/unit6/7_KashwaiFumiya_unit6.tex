...
各ステップの収率は\textbf{1}→\textbf{2}: 56\%、\textbf{2}→\textbf{3}: 48\%、\textbf{3}→\textbf{4}: 26\%であった。IRスペクトルおよび\ce{^1H} NMRスペクトルの帰属結果は、チャート同様にAppendixに示した。IRは各目的化合物の存在を支持した。融点測定値は、どの化合物もほぼ一定の融点を示し、かつ文献値と概ね一致した。図\ref{NMR_6-1-3}では化合物\textbf{4}に由来するピークのみが観測されており、高純度で合成することができたと考えられる。
