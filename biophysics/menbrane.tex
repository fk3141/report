...
オームの法則より、
\begin{equation}
    g_{\text{K}\infty} = \frac{I_{\text{K}\infty}}{V_{\text{C}}-{E_\text{K}}}
\end{equation}

$I_{\text{K}\infty}$の実測値として、次の読み取った値を用いた。

\begin{table}[htbp]
  \centering
  \begin{tabular}{rrrr}
    \toprule
    $V_c$ / mV & $I_{\text{K}\infty}$ / mA & $g_{\text{K}\infty}$ / mS& $n_\infty(V)$ \\
    \midrule
    25 &  5 & 48 & 0.870 \\
    60 & 10 & 71 & 0.963 \\
    85 & 13 & 79 & 0.987 \\
...
(b) 
\begin{equation}
    n_{\infty}(V) = \lr{\frac{g_{\text{K}\infty}}{\bar{g_{\text{K}}}}}^{\frac{1}{4}}
\end{equation}
より、$n_{\infty}(V)$は上の表の通りになる。

(c)
\begin{equation}
    I_{\text{K}}(t) = \lr{1- e^{-t/\tau_n}}^4
\end{equation}
より、$t_{1/2} = -ln{\lr{1-\frac{1}{\sqrt[4]{2}}}}\tau_n \simeq 1.84 \tau_n$とすると、$I_{\text{K}}(t_{1/2}) = \frac{1}{2}I_{\text{K}\infty}$となる。

グラフより$t_{1/2}$を読み取ることによって、次の表のように求められる。ただし
\begin{align}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{rrrrrr}
    \toprule
    $V_c$ & $n_\infty(V)$ & $t_{1/2}$ / msec & $\tau_n$ / msec & $\alpha_n$ & $\beta_n$ \\
    \midrule
    25  & 0.870 & 2.500 & 1.359 & 0.641 & 0.095 \\
    60  & 0.963 & 1.800 & 0.978 & 0.985 & 0.038 \\
    85  & 0.987 & 1.500 & 0.815 & 1.211 & 0.016 \\
    100 & 1.001 & 1.100 & 0.598 & 1.674 & -0.002 \\
    \bottomrule
  \end{tabular}
\end{table}
